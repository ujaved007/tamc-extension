...
function PatrolLog(data) {
	...
	selectDropdownPatrol(document.querySelectorAll('select[name$="REQGUARD"]'), data.reqguard);
	fillTextField(document.querySelector('input[aria-labelledby$="SITENAME_LABEL"]', data.sitename));
	selectDropdownPatrol(document.querySelectorAll('select[name$="SITESTATE"]'), data.sitestate);
